...
@Test func remoteHelperBatchesCannotExpandAuthority() throws {
    let url = "https://github.com/a/b.git", oid = String(repeating: "a", count: 40)
    for phase in ["list", "list for-push", "fetch", "push"] {
        let command = phase == "fetch" ? "fetch \(oid) HEAD" : phase == "push" ? "push \(oid):refs/heads/topic" : phase
        let wire = ["remote-helper", url, phase, "option dry-run true", "", command]
        let op = try #require(GitTransportOperation(wire))
        #expect(op.arguments(phase: "remote-helper", oid: "")?.suffix(3) == ["remote-https", url, url])
        #expect(op.classification == (phase.contains("push") ? .mutating : .localWrite))
        #expect(SecretGateProtection.readOnlyAndLocalWrites.allows(op.classification) == !phase.contains("push"))
    ...
    for option in ["cas", "atomic", "pushcert", "servpath"] {
        #expect(GitRemotePlan(["remote-helper", url, "list", "option \(option) true", "", "list"]) == nil)
    }
